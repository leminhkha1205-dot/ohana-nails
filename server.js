...
function buildVideoTagData(template, keywords) {
  ...
  // MOV/MP4 dùng QuickTime/Keys metadata. Ghi thêm XMP để phần mềm đọc metadata dễ hơn.
...
async function writeVideoMetadata(inputPath, outputPath, keywords, template) {
  await fsp.copyFile(inputPath, outputPath);
  await exiftool.write(outputPath, buildVideoTagData(template, keywords), ['-overwrite_original']);
}
...
async function handleMediaProcess(req, res, type) {
  ...
  try {
    for (let i = 0; i < req.files.length; i++) {
      ...
      const ext = type === 'video' ? (originalExt && ['.mov', '.mp4', '.m4v'].includes(originalExt) ? originalExt : '.mov') : '.jpg';
...
app.get('/api/health', (_, res) => res.json({ ok: true, app: 'Nail SEO Metadata App', version: '7.0.0', templates: Object.keys(TEMPLATES), features: ['image-seo', 'video-seo'] }));
...
app.listen(PORT, () => console.log(`Nail SEO Metadata App v7 running on port ${PORT}`));
